...
window.Diamonds = function(customOptions) {
	// Implements : Events,
	...
	this.options = {
        ...
        itemWrap : Elements.from('<div class="diamond-box-wrap"><div class="diamond-box"><div class="diamond-box-inner"></div></div></div>'),
        rowWrap : Elements.from('<div class="diamond-row-wrap"></div>'),
        rowUpperWrap : Elements.from('<div class="diamond-row-upper"></div>'),
        rowLowerWrap : Elements.from('<div class="diamond-row-lower"></div>'),
        diamondWrap : Elements.from('<div class="diamonds"></div>'),
...
Diamonds.prototype._createOverrideCss = function() {
    ...
    var style = Elements.from('<style type="text/css"></style>');
    style.set("html", css);
...
Diamonds.prototype._removeOverrideCss = function() {
	if(this.styleElement) this.styleElement.destroy();
};
...
Diamonds.prototype.setOptions = function(customOptions, redraw) {
    ...
    if(customOptions !== null && typeof customOptions === "object") {
        ...
        this.options = Object.merge(this.options, customOptions);
...
Diamonds.prototype._getScrollbarWidth = function() {
	if(!isNaN(parseFloat(this.options.scrollbarWidth) && isFinite(this.options.scrollbarWidth) && this.options.scrollbarWidth >= 0) {
        return this.options.scrollbarWidth;
    }

    var $inner = Elements.from('<div style="width: 100%; height:200px;">test</div>'),
        $outer = Elements.from('<div style="width:200px;height:150px; position: absolute; top: 0; left: 0; visibility: hidden; overflow:hidden;"></div>').append($inner),
        inner = $inner[0],
        outer = $outer[0];
...
Diamonds.prototype._renderHtml = function(rows) {
    ...
    for(var i = 0; i < rows.length; i += 2) {
        ...
        for(var j = 0; j < rows[i + 1].length; j++) {
            ...
            $(rows[i + 1][j]).wraps(this.options.itemWrap);
        }
    }

    wrap.setStyle("min-width", this.options.minDiamondsPerRow * this.options.size);

    return wrap;
};

Diamonds.prototype.draw = function(first_argument) {
	if(this._triggerEvent("beforeDraw")) return;

    this._emptyElement(this.wrapElement);

    // Zepto version
    var width = this.wrapElement.getSize().x 
        - parseFloat("" + this.wrapElement.getStyle("padding-left"))
        - parseFloat("" + this.wrapElement.getStyle("padding-right"))
